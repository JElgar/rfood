...
\documentclass[ oneside,% the name of the author
                ...
                % the dissertation    title (which cannot be blank)
                     title={Bidirectional transformer between functional and \\ object-oriented decomposition in Rust},
...
The ``Expression Problem" outlines the complementary expressiveness of object oriented and functional decomposition. Functional decomposition enables easy extension of functions for a datatype, whilst object oriented decomposition enables easy extension of datatype cases. With modern programming languages becoming increasingly multi paradigm developer now have the choice of which style to choice. With either there are inherently trade offs. This paper formalises translations between these two paradigms in Rust. It implements the transformer and integrates it as a refactoring tool in the popular IDE VS Code. With this developers can automatically transform their code between these decomposition styles, thus overcoming the trade-offs of the original implementation style.
...
\noindent
\begin{itemize}
    \item Extend the FOOD calculus from \cite{food}, to encapsulate the Rust typing system.
    \item Based on the translations from \cite{food}, formalize translations between object oriented and functional decomposition in RFOOD. In which a set of required type translations and an additional translation stage to ensure the correct type of the expression.
    \item Created an implementation of the transformer for Rust and demonstrate its application on a variety of examples.
    \item Integrate the transformer into the VS Code IDE. 
    \item Extend the transformer beyond the bijective translations from \cite{golog}, to support a wider range of program inputs.
\end{itemize}
\end{quote}
...
I would like to thank Weixin Zhang and Meng Wang for their continued support throughout the project.
...
This project did not require ethical review, as determined by my supervisor, Meng Wang.
...
Functional and object oriented decomposition enable two distinct approaches to program design. Functional decomposition is a ``constructor-centric" \cite{golog} approach, which enables simple extensions of datatypes when adding constructors, yet more complicated extensions when adding destructors. Whereas object oriented decomposition is ``destructor-centric" and thus has the opposite trade-off. These trade-offs, as outlined in the well established ``Expression Problem" from \cite{phil}, have lead to wide range of research exploring whether a programming language which enables both types of extension without updating existing code is possible.

The work of Binder \etal in \cite{golog}, demonstrated a symmetry between these decomposition styles can be constructed. With this they developed a minimal language upon which automated translations can be applied. \weixin extended this work by demonstrating these principals can be applied directly to existing multi-paradigm languages. They formalized the translations on a minimal calculus based on Scala, and demonstrated its practical applications with a Scala implementation.

In this paper we hope to demonstrate that, the approach outlined in \cite{food} can be applied to other multi-paradigm languages. 
Rust has many differences to Scala especially in terms of its typing system. Hence, by
implementing the transformer in this language, we hope to demonstrate the true generality of these translations. Moreover with Rust's direct control over memory, we hope the concepts applied in this paper will reveal the additional requirements of transforming such a languages. As such we hope the findings in this paper can be applied to the development of transformers for other languages.

With the rapid adoption of functional and object oriented features in modern programming languages, developers are increasingly able to select the paradigm they use for a given program. Currently, switching between these styles requires lengthy refactoring and as such the value of a tool as outlined in \cite{golog} and \cite{food} is ever increasing. With this, in this paper we aim to increase the practicality of the tool, both extending the type of programs which it can translate as well as making it more accessible to developers by integrating into their existing workflows.

The work of both \cite{food} and \cite{golog} are based on bijective symmetry between concepts in object oriented and functional decomposition. We hope to show that translations beyond these direct mappings are possible and thus the types of program which can be translated are far greater.
...
With this the concerte goals of this paper as as follows:

\begin{enumerate}
\item Create an minimal calculus which encapsulate the concepts of the Rust typing system.
\item Create a formalization of translations between object oriented and functional decomposition within this calculus.
\item Implement these translations to create an automated transformer for Rust.
\item Create an IDE tool to allow seamless integration of the tool into developer workflows.
\end{enumerate}
...
\section{Decomposition Symmetry}
...
problem at hand." \cite{warburton}. 

In 1998, Wadler coined the phrase the ``Expression Problem", in \cite{phil} which referred to the creation of a programming language which could both extend a datatype in terms of its cases and its functions without the need for recompiling existing code. He argues that existing languages fall into one of two categories, functional languages where the cases of a datatype are fixed, whilst adding new functions is easy and object oriented languages where the functions are fixed but new cases can be added easily. 

Since then there have been many works focused on this trade off \cite{inproceedings}\cite{kris}\cite{golog}\cite{food}, with the ultimate goal of solving the expression problem and thus making it possible to perform both of these extensions in single language.

Binder \etal argue in \cite{golog} that programming languages should support object oriented and functional decomposition styles in a symmetric way. This would allow for automated translation between the two, thus allowing developers to switch to the preferred style depending on the extension. They propose a minimal language with bijective translations between the two decomposition styles and develop an IDE for the language with a transformer included as a refactoring tool.
The primary limitation of this work as stated in the paper is ``current programming languages are usually not symmetric and hence make it unnecessarily hard to switch the decomposition". As such applying this approach to current industry standard languages has limited scope.

% This statement is the motivation for this paper. Programmers are increasingly working within multi-paradigm projects and are expected to pick the ``best" features from a paradigm depending on their task at hand. Selecting the ``wrong" style or if requirements change, can be a costly as refactoring between styles is a time consuming process.

% With this it is clear that both styles have valuable roles, and there is value in
% there is value in a tool which can automatically switch between features in these paradigms.

In \cite{food} \weixin extend this approach by mapping the principals of \cite{golog} to existing multi-paradigm languages. \weixin create a minimal calculus, ``FOOD", which is based on the popular systems programming language Scala. Using this syntax, they define a set of type-directed transformation rules, to formalise transformations between Cook's pure OOP decomposition style, outlined in \cite{food} and functional decomposition style.
...
% The work of \weixin \cite{food}, demonstrates, that practical equivalence can be drawn between certain features and as such an automatic transformer, in some capacity, can be created. The goal of this paper, is to both replicate the functionality of that paper in a different language, but also to extend the transformer to support a wider range of inputk

% In \cite{golog} Binder \etal argue that programming languages should support these decomposition styles in a symmetric way. This allows for automated translation between the two 

% In ``Decomposition without Regrets" \cite{food}, \weixin proposes a framework for a bidirectional transformer between these two decomposition styles. \weixin introduces a syntax called "FOOD calculus", which is heavily based on Scala. 

% These rules outline the fundamental transformations required to convert between the two styles within FOOD calculus, including transforming the top level types (datatype and traits) and their associated functions (generators, constructors, destructors and consumers).

% % \weixin uses type directed rules to formalize the transformation to perform.

% In current popular languages, this symmetry between feature is limited...
...
The $Self$ type has been kept separate from the other types in the syntax as it can only be used in destructors. The match statement is also not included as a standalone expression as pattern matching is currently only supported in constructor match statements. 
Within the rules, $\overline{\text{overlines}}$ are used to denote a sequence. Note variables with an overline are distinct from those which are not, i.e. $x$ is different from $\overline{x}$.
There are some additional restrictions on expressions and definitions which are not represented in the syntax, these are outlined throughout this section.
  ...
  \begin{displaymath}
    \begin{array}{l}
      \begin{array}{llcl}
        ...
        \text{Datatype}
         & Dt & \Coloneqq & \datatype {\dtn} {\ctr {\ctrn} {\fnArgs{x}{T}}}\\
        \text{Interface}
         & It & \Coloneqq & \interface {\itn} {Dtr}\\
        \text{Destructor}
         & Dtr & \Coloneqq & \fnDstr {\fnArg{self}{S}} {x: T} {T} \mid \fnDstr {\fnArg{self}{S}} {x: T} {T} = e  \\
        % \text{Constructor}
        % & Ctr & \Coloneqq & \ctr {\ctrn} {\fnArgs{x}{T}} \\
        \text{Generator}
        %  & gen & \Coloneqq & \class C {C(\overline{e})} {fun}\\
         & Gen & \Coloneqq & \class{\genn}{x}{T}{\itn}{\overline{Fun}}\\
        ...
        %  &   &           & \ifelseexpr{e_1}{e_2}{e_3} \\
         \text{Pattern}
         & P & \Coloneqq & \ctrCall{\dtn}{\ctrn}{x} \mid \_ \\
        %  \text{Modifier}
...
\begin{mathpar}
\inferrule[Gen2Ctr]
{ \overline{\transType {\itn} T {T'}} \\ \transtwoNT {} L {L'} \\ C = G} %  
{ \transtwoNT {}{\class {\genn} {x} {T} {\itn} {\overline{Fun}};L} {\ctr{C}{\overline{x: T'}}};L'}
\end{mathpar}

\begin{mathpar}
\inferrule[Dec2Csm]
{\fresh{d} \\ \overline{\genn} = \generator{\itn} \\ \transType{\itn}{T_{s}}{T_{s}'} \\ \overline{\transType{\itn}{T_{x}}{T_{x}'}} \\ \transType{\itn}{T}{T'} \\ \transMatchExpr{T_s'} {d} {e_d} \\ D = I \\ C = G
}
{\transtwoNTB{\itn}{\decD{self: T_{s}}{x: T_{x}}{T}}{
    \csmcase{\fnArg{d}{T_{s}'}} {\fnArg{x}{T_{x}'}} {T'} {e_d}  {\ctrPat{\dtn}{\ctrn}(\attrs(\genn))} {[self \mapsto d]\gdefinition{f}{\genn}}}
}
\end{mathpar}
...
{\fresh{d} \\ \transtwoNT{}{\decD{self: T_{s}}{x: T_{x}}{T}}{
    \csmcase{\fnArg{d}{T_{s}'}} {\fnArg{x}{T_{x}'}} {T'} {e_d}  {P} {e}}
}
{\transtwoNTB{\itn}{\fnDec{self: T_{s}, \overline{x: T_{x}}}{T}{e}}{
    \csmcase{\fnArg{d}{T_{s}'}} {\fnArg{x}{T_{x}'}} {T'} {e_d}  {P} {e}}; \matchcaseNO{\_}{[self \mapsto d]e}
}
\end{mathpar}
...
\inferrule[RefSelf2Ref]
{\dtn = \itn}
{ \transType{\itn}{\& Self}{\&\dtn}}

\inferrule[PntSelfDt]
{\dtn = \itn }
{ \transType{\itn}{\sim Self}{\dtn}}

\inferrule[DtrPntDt2It]
{\itn \in \ite \\ \dtn = \itn}
{ \transType{\itn'}{\sim_{dyn}\itn}{\dtn}}

\end{mathpar}

\begin{mathpar}

\inferrule[PtnIt2PtnDt]
{\itn \in \ite \\ \dtn = \itn}
{ \transType{}{\sim_{dyn} \itn}{\sim \dtn}}

\inferrule[RefIt2RefDt]
{\itn \in \ite \\ \dtn = \itn}
{ \transType{}{\&_{dyn} \itn}{\&\dtn}}
...
\inferrule[Ref]
{}
{ \transMatchExpr{\& Self}{d}{\& d}}

\inferrule[Pnt]
{}
{ \transMatchExpr{\sim Self}{d}{d}}

\end{mathpar}
\caption{Stage 1 OOP - Match expression transformation}
\label{fig:trans-match-expr}
\end{figure}
...
{\transtwoNT{D,C}{\csmcase{d: T_{s}}{x: T_{x}}{T}{e_d}{P}{e}} {\fnDec{\overline{self: T_{s}', x: T_{x}'}}{T'}{[d \mapsto self]e}}}
\end{mathpar}

\begin{mathpar}
\inferrule[Csm2Dec]
{\matchcase{\_}{e} \notin \overline{\matchcase{P}{e}} \\ \transTypeSelf{}{T_{s}}{T_{s}'} \\ \overline{\transType{\dtn}{T_{x}}{T_{x}'}} \\ \transType{\dtn}{T}{T'}}
{\transtwoNT{D}{\csmcase{d: T_{s}}{x: T}{T}{e_d}{P}{e}} {\fnDstr{self: T_{s}'}{x: T'}{T'}} }
\end{mathpar}

\begin{mathpar}
\inferrule[Csm2Fun]
{\matchcase{\_}{e} \in \overline{\matchcase{P}{e}} \\ \transTypeSelf{}{T_{s}}{T_{s}'} \\ \overline{\transType{\dtn}{T_{x}}{T_{x}'}} \\ \transType{\dtn}{T}{T'}}
{\transtwoNT{D}{\csmcase{d: T_{s}}{x: T}{T}{e_d}{P}{e}} {\fnDec{self: T_{s}', \overline{x: T'}}{T'}{[d \mapsto self]e}} }
\end{mathpar}
...
\inferrule[Dt2DynPnt]
{  \dtn \in \dt \\ \itn = \dtn  }
{ \transType{}{\dtn}{\sim_{dyn} \itn}}

\inferrule[DtPnt2DynPnt]
{  \dtn \in \dt \\ \itn = \dtn  }
{ \transType{}{\sim \dtn}{\sim_{dyn} \itn}}

\inferrule[DtRef2DynRef]
{ \dtn \in \dt \\ \itn = \dtn }
{ \transType{}{\& \dtn}{\&_{dyn} \itn}}

\end{mathpar}

\begin{mathpar}

\inferrule[Dt2SelfPnt]
{}
{\transTypeSelf{}{\dtn}{\sim Self}}

\inferrule[DtPnt2SelfPnt]
{}
{\transTypeSelf{}{\sim \dtn}{\sim Self}}

\inferrule[DtRef2SelfRef]
{}
{\transTypeSelf{}{\&\dtn}{\& Self}}
...
\begin{mathpar}
\inferrule[DecFn]
{\transSigmaCtx{, \overline{x: T}}{T}{e}{}{e'}{} {\leadsto}} %\\ {\trans t {\overline{T} \arrow T} {t'}} }
{\transThreeStmt{\fnDec{\overline{x: T}}{T}{e}} {\fnDec{\overline{x: T}}{T}{e'}}}
\end{mathpar}

\begin{mathpar}
\inferrule[DecCsm]
{\overline{\transSigmaCtx{, \overline{x: T}}{T}{\matchcase{P}{e}}{}{\matchcase{P}{e'}}{}{\leadsto}}} %\\ {\trans t {\overline{T} \arrow T} {t'}} }
{\transThreeStmtB{\csmcase{d: T_{s}}{x: T}{T}{e_d}{P}{e}} {\csmcase{d: T_{s}}{x: T}{T}{e_d}{P}{e'}}}
\end{mathpar}

\begin{mathpar}
\inferrule[GCase]
{ Def(C) = D::C(\overline{\fnArg{x}{T}}) \\ \transSigmaCtx{, \overline{x: T}}{T}{e}{}{e'}{}{\leadsto}}
{\transThreeStmt{\matchcase{D::C(\overline{T})}{e}} {\matchcase{D::C(\overline{x})}{e'}}}

\inferrule[WCase]
{\transSigmaCtx{}{T}{e}{}{e'}{}{\leadsto}}
{\transThreeStmt{\matchcase{\_}{e}} {\matchcase{\_}{e'}}}
\end{mathpar}
...
\caption{Stage 2 - Program and definition transformation rules}
...
\begin{mathpar}
\inferrule[Exp]
{\transSigmaInner{\sigma}{e}{e'} \\ \gdctx \vdash e' \rightarrowtail e'' \Leftarrow \sigma}
{ \transSigma{\sigma}{e}{e''}}
\end{mathpar}
...
\begin{mathpar}
\inferrule[Sel2App]
{ \transSigmaTT{\dtn}{e_1}{e_1'} \\ f \in \consumer{D} \\ e = \fncall{e_1, \overline{e_2}} \\ \transSigma{\sigma}{e}{e'} } %\\ {\trans t {\overline{T} \arrow T} {t'}} }
{\transSigmaInner {\sigma} {\methodcall{e_1}{\overline{e_2}}} {e'}}
\end{mathpar}

\begin{mathpar}
\inferrule[Sel]
{ \transSigmaTT{\itn}{e_1}{e_1'} \\ f \in \destructor{\itn} \\ \sig(f) = \fntype{\overline{T_{s}}}{T} \\ \transThreeStmt{e_1}{e_1'} \\ \overline{\transSigma{T_s}{e_2}{e_2'}}} %\\ {\trans t {\overline{T} \arrow T} {t'}} }
{\transSigmaInner {\sigma} {\methodcall{e_1}{\overline{e_2}}} {\methodcall{e_1'}{\overline{e_2'}}}}
\end{mathpar}

\begin{mathpar}
\inferrule[App2Sel]
{ \transSigmaTT{\itn}{e_1}{e_1'} \\ f \in \destructor{I} \\ e = \methodcall{e_1}{\overline{e_2}} \\ \transSigma{\sigma}{e}{e'} } %\\ {\trans t {\overline{T} \arrow T} {t'}} }
...
\begin{mathpar}
\inferrule[App]
{ \transSigmaTT{T}{e_1}{e_1'} \\ f \in \fn \cup \consumer{T} \\ \sig(f) = \overline{T} \rightarrow T \\ \overline{\transSigma{T}{e}{e'}} } %\\ {\trans t {\overline{T} \arrow T} {t'}} }
...
\label{fig:trans-stage3-expr-type}
\end{figure}

Having collected the global context, the first stage of the translation is to transform the user defined types, and their associated functions. These follow the rules outlined in \autoref{fig:stage2-trans-oop} and \autoref{fig:stage2-trans-fp}. In these rules, $\transtwoNT{}{e}{e'}$ represents the transformation of $e$ to $e'$ under the context of $\Gamma$. In some cases additional context is required to be passed between rules, which is represented with $\vdash_{c}$ where c is set of additional context variables. Specifically for FP, \rulename{Ctr2Gen}, \rulename{Csm2Dec} and \rulename{Csm2Fun} take a datatype, whilst \rulename{Case2Fun} takes both a datatype and constructor. In OOP, \rulename{Dec2Csm} and \rulename{Fun2Csm} take an interface.
...
However in FP there are two styles for types which take ownership of datatype variables, yet there is only one style for traits in OOP. In the FP datatypes can be used as a type directly or behind a pointer, whilst in OOP traits can only be used behind dynamic pointers. Thus when converting from FP to OOP both styles are converted to dynamic pointers as shown in \rulename{DtPnt2DynPnt} and \rulename{Dt2DynPnt} rules in \autoref{fig:trans-type-fp}, where as for OOP to FP a decision has to be made of which style to used.
...
Another consideration of destructor methods is the \verb|Self| type. In trait definitions this is a reference to the (unsized) trait, whilst in implementation block it is analogous to the struct type. In both cases this does not require a dynamic reference. For the purposes of the transformer, when a reference to Self is used this is analogous to a reference to the trait, where as when the Self type is used directly, this is analogous to a dynamic reference to the trait. For clarity rules for the transformation of Self have been included explicitly in \autoref{fig:trans-type-oop} and \autoref{fig:trans-type-fp}. In FP a special syntax $\transTypeSelf{}{}{}$ is used to indicate the first argument of the consumer is being transformed, and thus it should be converted to the Self type.


\subsection{Transformation of Type Definitions}

The first stage of this process is to transform the user defined types themselves. These transformations follow the \rulename{It2Dt}, \rulename{Dt2It}, \rulename{Gen2Ctr} and \rulename{Ctr2Gen} rules which are heavily based on the matching rules from \cite{food}. However, these rules have been adapted to support the Rust type translations. With this step the types are converted to the new decomposition styles and the types of the attributes are updated.
...
For OOP to FP the following steps are followed for each trait as outlined in \rulename{It2Dt} and \rulename{Gen2Ctr} from \autoref{fig:stage2-trans-oop}:
\begin{enumerate}
...
For FP to OOP the following steps are followed for each enum, as outlined in \rulename{Dt2It} and \rulename{Ctr2Gen} from \autoref{fig:stage2-trans-fp}:
\begin{enumerate}
...
The steps for transforming the associated functions in OOP, as outlined in \rulename{Dec2Csm} and \rulename{Fn2Csm} from \autoref{fig:stage2-trans-oop}, are as follows.
...
\begin{enumerate}
    ...
    \item The body of the consumer is set as a match statement, which matches on the transformed \verb|self| parameter. In Rust (outside of the nightly compiler) a pattern match cannot occur directly on box, as such, if the \verb|self| parameter has a pointer type then it is deferenced in the match expression. Equally if the type of the \verb|self| argument is a borrow, then to align with the ownership of the destructor a borrow of this parameter should be used as the match expression. The mapping of match expression to \verb|self| parameter types are denoted by $\transMatchExpr{T}{d}{e}$, where $T$ and $d$ are the type and name of the \verb|self| parameter, and $e$ is the match expression which should be used. The rules are outlined in \autoref{fig:trans-match-expr}.
    A match arm is then created for each of the variants of the datatype, with all of the fields of the datatype included in the pattern match.
...
For FP  the following steps are followed, as outline by \rulename{DecFn} and \rulename{DecCsm} from \autoref{fig:stage2-trans-fp}.
...
For both transformation direction, any expressions which are not transformed during the first stage by the provided rules, still require the types to be updated. For example if a top level function (which is not a consumer) is included the types of all the parameters and the return type should be updated as outlined in the type transformation rules. This is also required for any destructors and their implementations which are not transformed. Note beyond the current implementation there are many other places explicit types can be assigned, such as let expressions and closures. These are currently not supported.
...
The transformations for programs and definitions are provided in \autoref{fig:trans-stage3-prog}. Expression transformations are outlined in \autoref{fig:trans-stage-3-expr}. $\transSigma{\sigma}{e}{e'}{}$ states that expression $e$ is transformed to $e'$ of required type $\sigma$ under the context of $\Delta$ and $\Gamma$. All expressions are transformed by the top level rule \rulename{Exp}. This then uses the subset of inner expression transformation rules, denoted with $\twoheadrightarrow$. The result of this inner transformation is then passed to the type transformations from \autoref{fig:trans-stage3-expr-type} denoted by $\rightarrowtail$.
...
\rulename{Gen2Ctr} and \rulename{Ctr2Gen} outline the transformations between generators and constructors. In Rust structs (generators) are in the global scope, where as by default enum variants (constructors) are nested within enums. For this reason when transforming generators into constructors, the path is extended with the datatype, for example \rust{Dimmer} to \rust{Light::Dimmer}. For FP this transformation is done in reverse, removing the datatype from the path.
...
\subsubsection{Block Expressions}
 
When transforming a block expression all nested expressions should be transformed in order. The expected return type of all expressions should be none except the final expression which should be equal to the expected return type of the block. The $\Gamma$ context should also be updated for each let expression in the block.

% \begin{mathpar}
% \inferrule[Block]
% {\transSigma{\sigma}{e_1}{e_1'} \\ \transThreeStmt{e_2}{e_2'}} %\\ {\trans t {\overline{T} \arrow T} {t'}} }
% {\transSigmaInner {\sigma}{e_1; e_2} {e_1'; e_2'}}
% \end{mathpar}
...
\section{Applications of the Rust Transformer}

A wide range of examples have been produced to demonstrate the functionality of the transformer. The full set of examples have been published here \url{https://github.com/JElgar/rfood}. A suite of units tests have been created to validate the examples and their outputs.

To demonstrate the value of the transformer, we begin by discussing the extension of a program from \cite{oopdesign} which models shapes. In a practical setting, the objective value of the transformer is its ability to increasing the efficiency of developers. It achieves this by allowing developers to write and extend code in the optimal decomposition style depending on the task at hand. 

The shape program begins with the following \rust{Shape} trait which outlines a set of destructors. 
...
Notice the whole implementation of the type is contained in a single place, and no alterations to the original \rust{Shape} type are required. However, when extending the example to support additional destructors the complexity is increased. For example in order to support a side count method, firstly the method would have to be added to the trait, then each generator of the trait would have to be extended to implement this new method. In larger projects it is likely there are many generators across multiple files.

To simplify this process the transformer can be used. By first converting the program into functional decomposition style, the additional method can be added as a single consumer definition as follows
...
Much like adding types in object oriented decomposition style, adding this consumer definition is an isolated process and thus no changes beyond adding the definition are required. With this, in this example, it is clear that functional decomposition is preferred for adding functions, whilst object oriented is preferred for adding types. Using the transformer allows developers to switch between these styles without a lengthy refactoring process. With this they can select the optimal style for the task and thus increase their efficiency. The impact of this tool has a compounding effect as program sizes increase, as the burden of finding each generator of an interface or consumer of a datatype increases. Moreover with the additional complexity of the Rust typing system, if a manual refactor is performed developer also have to consider the updated types, further increasing the complexity of this task. This tool avoid these issue by automating this process allowing developers to automatically and instantly switch between styles.

The VS Code extension allows developer to integrate this tool directly into their workflow. With the tool available in a familiar setting alongside other refactoring tools, the barrier to entry for using the tool is significantly reduced. The following video, \url{https://youtu.be/Z9bvl1Odcds}, demonstrates the implementation of the extensions to the Shape example discussed in this section. By using the VS Code extension, the developer is able to efficiently implemented both the additional shape and the additional function, switching between decomposition styles as required. The current extension is a proof of concept and has the potential to be greatly extended in the future, such as allowing developer to select types that they wish to transform or configuring different styles of output for the transformer.

\section{Limitations of the Rust Transformer}

The previous section discuses the practical applications of the transformer, demonstrating even at this early stage of development, the tool can have real positive impact on developer workflows. Despite this, there are still many limitation of the transformer.

\subsubsection{Syntax}

Although a wide range of statements and expressions are supported in the current Rust implementation, there are many which have limited or no support. Examples such as types in let expression, unnamed enums and closure would require extensions in the transformer to be supported. In most cases extending the transformer to support these addition concepts is trivial and the implementation is written such that these can be included easily. With the transformer separated into two stages additional rules are always required for the second stage, whilst rules for the first stage are only required if explicit types are used, such as in closures or let expressions with type assignments.

\subsubsection{Type Inference}

Type inference for the current implementation is achieved using the typing rules from, 

Although a practical solution for the limited RFOOD syntax, in reality this approach can be naive. As outlined in the previous section, certain expression types have limited to no support. Moreover, without support for casting and overloading of operators, this approach can be very limited.

Although extending this approach to support additional concept is possible, for practical applications an alternative approach would be to use the built in type inference from the rust compiler. The rust compiler parses the source code (including all dependencies) and converts it to an AST, then to a HIR (High-Level Intermediate Representation) and then finally a THIR (Typed High-Level Intermediate Representation), where the typing information is added. It collects (and checks) these types using the \verb|rustc_typecheck| package which can be exposed using the nightly compiler. Currently the documentation for this package is very limited and the API for all of \verb|rustc| remains unstable. For this reason this approach was not developed for this version of the compiler, but could create some tangible improvements to the type checking in the future. Another improvement of using this crate is the type checking could automatically track with the version of rust the developer is using. This means as rust add or changes features in the language the type inference of the transformer would not have to be updated directly.
 
\subsubsection{Multiple File Support} 

One of the most significant limitations of the current implementation is the lack of support for transforming projects which span multiple files. This results in developers being unable to import any functionality in their project and being able to separate the project into different modules and libraries. Ultimately this limits the practical application of the transformer to small projects.

Although this limitation also exists for the Scala transformer, the impact is far greater for Rust. In Rust, given that types are transformed, the signature of imported function is always required to be known so that expressions can be updated. In Scala, as types are not impacted by the translations the signature of the function does not need to be considered if it remains unchanged throughout the translations. As a result importing functions into the Rust transformer results in the transformer failing, whereas for the Scala implementation the imported program will simply remain unchanged.

Extending the project to support multiple files is undoubtedly possible although presents some unique challenges. Firstly the global context will have to be collected for all files and scoped based on import statements. With this the signatures of imported function will also be imported and thus the transformer will be able to use features from other files. However another consideration when transforming multiple files is, it is possible that generators of traits or consumers of datatype are defined across multiple files. In this case the location of the transformed definitions will have to be consistently selected.

Note supporting multiple files also applies to built in definitions. In the current implementation, built in constructs such as vectors and iterables cannot be used as the signatures are unknown. Extending the implementation to import definitions from other files could resolve this issue.

\subsubsection{Shadowing}

Another limitation of the transformer is it currently has no support for shadowing. As a result if a variable is named the same in a datatype as function consumer, or in a generator and a destructor, this can result in the incorrect variables being referenced. This is outlined in the rules by specifying that variable must be fresh, but has not been reflected in the implementation. To handle this a renamed operation would have to be performed to update every reference to one of the variables.

\subsubsection{Scoping}

Currently the recursive approach of second stage of the transformer, in conjunction with the prepossessing parse to collect top level definitions, ensures that the environment context is always accurate. At each recursive step a cloned copy of the current environment is passed in. This can then be mutated throughout this stage (such as for block expression where delta is extended by let assignments), these mutations are provided to nested stages but they have impact on the context of higher scopes.

For the first stage however, the ``global context" is only collected at the beginning of the transformation. In formalization the syntax is restricted such that definitions can only be created at the top level and such this approach is correct. However, in Rust, definitions can be created in nested blocks, and such to have proper support of scoping Gamma should be collected along side the other translations, ensuring the correct datatypes are in scope.  
Note, however, the top level types must be collected before any other types are transformed, as types defined after a function can still be referenced in that function.

\subsubsection{Simplifications}

There are many examples where the outputted code could be simplified, for example: 

\begin{itemize}
    \item The current implementation exposes all fields in a pattern match. Instead it should only expose those which are actually used. This could be determined by collecting all field attribute calls on the \verb|self| parameter. Using this the fields which are not reference inside the match expression could be ignored using the \rust{..} syntax.
    \item Equally in the current implementation, does not support the simplified syntax for assigning named fields, to a variable with the same named. With this, if a variable x is in scope and a struct, S, has a field called x, the field can be assigned directly with the following syntax \rust{S{n}} rather than the traditional syntax \rust{S{n: n}}.
\end{itemize}

\subsubsection{Limitation from Rust Language}

The transformer is also limited in some capacity as a result of the Rust language.

\paragraph{Inheritance.} In Rust inheritance has limited supported. As discussed in \autoref{sec:background-inheritance} default implementations in Rust are limited in what they can return. This results in some FP constructs which can not be translated.

\paragraph{Macros} Macros have very limited support in the transformer. Currently each input to a macro is assumed to be an expression, which can be transformed directly. This is a very limited approach as macros expose the AST of the input directly and therefore it is not possible to predict the functionality. Extensions to this approach are possible, such as extracting information from declarative macro patterns, but a generic solution for all macros is unfeasible. 

\section{Type Transformations}

The primary contribution of this paper, is the novel extension of the translations from \cite{food} and \cite{golog} to support Rust's unique (within mainstream industry languages) typing system. To support the Rust typing system, the translations required significant extensions. The primary difference, arises as a result of how Rust handles dynamically sized types. As traits fall into this category, it requires traits to always be behind a dynamic references or pointer.
Given datatypes can be used as types directly, when transforming a datatype, whenever it is used as a type must also be updated. Similarly whenever a trait is used as a type, the dynamic token must be removed.
The approach outlined in this paper splits the transformer into two stages. The first updates the types, whilst the second is responsible for transforming the types of expressions, thus decoupling these concerns.
Given the type transformations are not injective, design decisions are made throughout for what transformations should be applied. By decoupling the expression updates from this stage, it allows these design designations to be updated easily as these changes do not impact the second stage of the transformer.

An alternative approach would be to compile the translations into a single stage, where both types and expressions are updated in a single parse. Although resulting in a simpler formalization this would lead to a much closer coupling of type and expression translations and thus be more complex to implemented and update.
Moreover with this approach additional considerations would be required to handle the collection of top level definitions. In the current implementation, both the global context and the environment (for top level definitions) are recollected after the first stage and thus expose the updated signatures of each type. When combining the stages, the order in which translations should be applied would become significantly more complex.

% Example working
An implementation of the Set from \cite{okasaki1999purely}, has been created in object oriented decomposition style. In order to further explore the typing translations, we provide some extracts from the translation of this example into functional decomposition. 

Firstly, we look at the translation applied to the signature of the \rust{union} destructor. 
In the OOD style input program the signature is as follows:

\begin{minted}{rust}
fn union(self: Box<Self>, s: Box<dyn Set>) -> Box<dyn Set> 
\end{minted}

and once translated the signature becomes:

\begin{minted}{rust}
fn union(set: Set, s: Set) -> Set
\end{minted}

This demonstrates the value of applying custom translations to function signatures. Both the pointer to the \rust{Self} type and the dynamic pointers to the \rust{Set} trait are updated to use the Set datatype directly. This creates a simple  and clean interface for the union constructor in FD style. 
An alternative correct translation would be to use the \rust{Box<Set>} type opposed to using the \rust{Set} directly (or even any combination of \rust{Box<Set>} and \rust{Set} types). However as a result, the interface of the union destructor becomes less clean and a mapping between the, arguably most natural, implementation in FD and the required implementation in OOD is lost.

Outside of the destructors, the other style of type translations was selected. For example in the \rust{Union} struct, the two sets are transformed from dynamic boxes to boxes. Note in this case a \rust{Set} type could not be used directly as this would create a recursive type definition. However, this style was used throughout the translations in order to minimise the effect of the transformer throughout. Given the transformer is unaware of how other aspects of the code are used, minimising the impact of the changes was deemed optimal.

In some cases however is this not the case. For let expressions, the current implementation of the transformer sets the expected type of the expression to any. As such the type of the assigned variable can change as a result of a translation. This approach was chosen, as in a similar vein to above, this minimises the impact of the transformer, as rather than predicting changes to assignments that may be required, it instead does type corrects as they are.

This manifests in the \rust{demo} function for the \rust{Set} example, which contains the following:
...
Here two sets are created, \rust{s1} and \rust{s2}. They are both wrapped in a \rust{Box} so that they can be used with \rust{union} function. When this extract is transformed they following is produced
...
With the outlined approach the \rust{s1} and \rust{s2} assignments remain the same and as such, in order for these variable to be used with the transformed \rust{union} consumer, they must be deferenced. Given the variable the ownership of these variables is transferred to this function, the variable cannot be used again and therefore are never used as a \rust{Box}. A simpler implementation would assign the \rust{s1} and \rust{s2} variables to the \rust{Set} directly and therefore the additional indirection in the call to the \rust{union} function would not be required.
This highlights the limited nature of the current approach to typing. Current a small set of concrete rules are used to determine which type translations and expression type translations should be applied. There is potential for future work to extend this implementation to focus on an ``optimal" translation which minimises superfluous indirection.

% Each reference pointer to the trait is updated to directly use the transformed datatype. This creates a simple implementation  of the union 
% Outside of this
% the $s$ argument and the return type are both dynamic boxes. As such when translated, the type of the argument is updated   


% % Discuss how the destrucor singature are handled differently and why its nice there is a mapping between simple examples
% % Say however it does add complexity

% During the first stage of the translation, the signature of the \rust{eval} function is updated from \rust{}
% %An alternative approach to this, would be to define concrete translations for constructor and destructor, updating each rule to update expression to use new types. 
% % For example a constructor of a datatype would be updated to always be wrapped in a pointer when converted to a trait. Then any reference to the type could simply be updated to use this new type.
...
% Although the transformer creates transformations that are correct, it is not guaranteed that the transformations are optimal. Optimal in this sense refers to two things, minimising indirection operations and the speed of the transformed code. The type simplifications outlined in the implementation go some way to minimise the use of indirection where it is not required. However in some cases, it may be possible to improve the solution further. For example, the following extracts are taken from an expression example which is provided in full here TODO add to abstract \autoref{}. 
...
% In terms of speed of the outputted code, it is unlikely the transformation will have a significant impact. The transformer does not support the clone operation as borrowing introduced by the developer is maintained as a result of the translation. With this is 
...
One of the foundational concepts of the previous work on this topic is the bijective nature of the translations \cite{food}\cite{golog}. This one to one mapping between concepts, allows translations to be performed without information loss and in a predictable manner (regardless of configuration or implementation decisions within the transformer). When applying these translations to Rust however, this bijective mapping is not possible. As discussed throughout \autoref{chap:execution}, two types in functional style map to a single type in object oriented style. As a result, without restricting the input style which the programmer can use, an injective mapping is not possible. Moreover, beyond the RFOOD syntax, when implementing the Rust transformer other concepts such as visibility also lacked symmetric mappings.

With this a significant limitation of the Rust transformer is it is not self-inverting. This means when applied to practical applications, although the functionality may remain the same, design decisions of developers may be lost. Moreover, in some cases the functionality will not be maintained. For example, if a struct which is a generator, is set private but implements a public trait, after the translation it will become public. This arises as visibility of datatypes in Rust is less configurable than publicity of generators.
as the translations are extended to support additional features, it is unlikely symmetry between the paradigms will be observed and as such more information loss will occur.

To mitigate the impact of this, it may be possible to use some additional meta data alongside the translations to store additional information about the design.

As such for practical implementations it is likely information will be lost as a result of translations. There is potential for address this issue by storing additional meta data, along side translations, however this is left as future work.  

The symmetry between decomposition styles offers many guarantees about the translations and makes it far easier to formalize the correctness of translations. However practical languages go far beyond the limited syntax for which bijective translations can be created.
For the transformer to become a practical tool, it must support a wider range of inputs and style. With this the question arises, whether mappings can be created between other language concepts.

The Rust implementation, already demonstrates this is possible, with the type translations and visbility we see a set of non-injective translations which correctly transform programs. With the acceptance that information can be lost between translations, and thus reducing the complexity of the problem, significantly more programs can be transformed. 

In this paper, we demonstrate that mutable destrutors can be transformed into a consumer that does not mutate the state.

The transformer from \cite{food} is based on the correspondence between object oriented and functional decomposition within the FOOD syntax. This results in a very restrictive tool, as developers have to write code in format

This correspondence ensures that transformations are consistent and that they are self inverse, meaning applying the transformer twice to a given input, returns the same input.
However these strict equivalences cannot be drawn for all concepts in Rust.

% TODO Injectivety  stuff
% TODO Types
...
% TODO Multiple traits for one class 
...
The transformation rules outlined in this paper are based on those in FOOD and therefore clear equivalences between them can be drawn. For example \autoref{fig} gives the \rulename{Gen2Ctr} rule from FOOD, which maps closely to \rulename{Gen2Ctr} created for the Rust transformer. These equivalences are evident throughout the transformer, suggesting the foundations of the transformer can be applied to other languages.
