...
\weixin's transformer focuses on the transformations between Pure OOP style from \cite{cook} and FP style. However this requirement to use pure OOP style can be very restrictive for the types of OOP programs that can be transformed.
A common style for object oriented programming is to include methods that mutate the state of the class it self. An example is the getter and setter pattern where one functions on a class is responsilbe for returning a value and another mutates it, as shown in \autoref{fig:mutable-example}.

In rust, variables are immutable by default. On line 3 of the example, the \verb|mut| token before self, makes self mutable. This can then used in the implementation on line 12 to set the brightness of the Dimmer. 

% Horrible sentence
In FP, mutability is traditionally avoided. This can have many benefits such as making functions easier to test as no side effects are possible. Therefore to transform this kind of method, the mutations should be wrapped within the method itself and such the method should return a new instance of the self type.

This transformation is performed as follows:
\begin{itemize}
    \item When transforming a destructor of a trait, check if it is mutable and if so:
    \item Create a let expression to reassign the self type to a mutable clone
    \item Replace all field assignments on self with field assignments on the new mutable clone
    \item Return the mutated clone from the method
    \item Update the signature to reflected the new return type
...
The goal of this section is to evaluate the transformer. It will achieve this by discussing case studies that the transformer can and can't transform and explore why this is the case. It will compare the Rust implementation with the Scala implementation in \cite{food}, and discuss future improvements that are possible for the transformer. 
