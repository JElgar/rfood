...
\paragraph{Consumer.} Consumers are functions that operate on a datatype. In Rust these are functions, with the first argument being a Datatype.
\paragraph{Destructor.} A destructor is a function that operates on an interface. In Rust these are methods defined on the traits, and then mus either have a default implementation (in the trait) or an implementation in the generators impl of the trait.
...
The environment stores the current types of variables on the stack. In the Scala implementation a type is stored as string, however in the Rust implementation it is stored as a \verb|DeltaType| struct. This allows the encoding of both the type name stored as an Ident (a simple wrapper around String provided by \verb|syn|) and the reference type. 

The reference type is currently implemented as an Enum, with a variant for Box, Reference and None. These reference types are later used to transform the types of expressions as required.
...
\subsection{Item Transformations}
%TODO Talk about this
\label{sec:item-trans}

After the type transformations are completed a set of recursive functions are used to transform the remaining statements and expressions. These functions require, not only the global and environment context but also the expected return type of the expression and the transform type (direction). 
The return types are an enum with the following variants:
...
\paragraph{FP to OOP.} If a consumer contains a wildcard match, then the return type of the arm must be checked. If it returns a simple (Sized) type, then a default implementation of the trait can be created and and implementation in the impl block is not required. However if the return type is not sized then a default implementation cannot be used. In this case a copy of the wildcard expression is created for each of the impl blocks. 
...
\subsection{Rust Types}

\subsection{Generics}

\subsection{Mutability}

\weixin's transformer focuses on the transformations between Pure OOP style from \cite{cook} and FP style. However this requirement to use pure OOP style can be very restrictive for the types of OOP programs that can be transformed. A common sytle for object orieneted programming is to include methods that mutate the state of the class it self. An example of this is shown in \autoref{}
