...
The transformations for programs and definitions are provided in \autoref{fig:trans-stage3-prog}. Expression transformations are outlined in \autoref{fig:trans-stage-3-expr}. $\transSigma{\sigma}{e}{e'}{}$ states that expression $e$ is transformed to $e'$ of required type $\sigma$ under the context of $\Delta$ and $\Gamma$. All expressions are transformed by the top level rule \verb|EXP|. This then uses the subset of inner expression transformation rules, denoted with $\vdash_i$. The result of this inner transformation is then passed to the type transformations from \autoref{fig:trans-stage3-expr-type} denoted by $\vdash_t$.
...
A wide range of examples have been produced to demonstrate the functionality of the transformer. The full set of examples can be here \url{https://github.com/JElgar/rfood}. A suite of units tests have been created to validate the examples and their outputs.

\subsubsection{Shape}

To demonstrate the value of the transformer, we begin by discussing the extension of a program from \cite{oopdesign} which models various shape. The primary objective of the transformer is to increasing the efficiency of developers. It achieves this by allowing developers to write and extend code in the optimal decomposition style depending on the task at hand. 
