...
In both cases the first stage is to collect the global context, delta. This is used throughout the transformations to prevent repeated searches for items.

The next stage is the transformation of the user defined types (either datatypes or traits). With this stage the associated destructors/consumers are also transformed both updating the signature and the body. Implementations of destructors are converted into a match arm in a consumers, whilst consumers match arms are converted into  destructor implementations.

With the type transformations completed a additional parse over the code is completed to transform all other expressions, including the output of the type transformations. This transforms any references to transformed function and methods, and updates the types of expressions which have been changed as a result of the transformations.

\section{Syntax}

In \cite{food} \weixin uses a syntax called FOOD to formalize the transformation rules. In order to formalize the transformations in Rust, the following extensions to the FOOD syntax are required. Primarily two additional types are added, $\sim T$ and $\& T$ where T is type. These represent a pointer and a borrow respectively. Note for this report only the Box pointer is considered, but future extensions of this work could include other pointer types. The $\sim e$ and $\& e$ expression are then also added in order to create pointers and references of the expression $e$. The additional $*e$ expression can then be used to dereference these types. This extended syntax has been called RFOOD and is given in full in \autoref{fig:syntax}.
  ...
  \begin{displaymath}
    \begin{array}{l}
      \begin{array}{llcl}
        ...
        \text{Definition}
        & Def & \Coloneqq & Dt \mid It \mid Ctr \mid Gen \mid Csm \\
        \text{Datatype}
         & Dt & \Coloneqq & \sealed D\\
        \text{Interface}
         & It & \Coloneqq & \interface D {Dtr}\\
        \text{Destructor}
         & Dtr & \Coloneqq & \textcolor{red}{\destr \& x \mid \destr \sim x}\\
        \text{Constructor}
         & Ctr & \Coloneqq & \ctr C D\\
        \text{Generator}
        %  & gen & \Coloneqq & \class C {C(\overline{e})} {fun}\\
         & Gen & \Coloneqq & \class C D {Fun}\\
         \text{Consumer}
         & Csm & \Coloneqq & \textcolor{red}{\csmcase \& e} \\
         &     &           & \textcolor{red}{\mid \csmcase{}{e}}\\
        %  \text{Body}
        %  & body & \Coloneqq & \overline{P \Rightarrow e} \mid e\\
         \text{Declaration}
         & Dec & \Coloneqq & \dec x\\
         \text{\st{Function}}
         & \text{\st{Fun}} & \text{\st{$\Coloneqq$}} & \text{\st{Dec = e}}\\
        \text{Expression}
         & e & \Coloneqq & x %\mid \overline{(x: T)} \Rightarrow t 
          \mid e_1.f(\overline{e_2})  \mid f(e_1)(\overline{e_2}) \mid C(\overline{e}) \mid \newSeq{C}{e} \mid \textcolor{red}{\sim e\mid \&e \mid *e}\\
         \text{Pattern}
         & P & \Coloneqq & C(\overline{x}) \mid \_ \\
        %  \text{Modifier}
        %  & m & \Coloneqq & \kwsealed \mid \kwabstract \mid \kwcase\\
        \text{Types}
        & T & \Coloneqq & D \mid \overline{T} \rightarrow T \mid \textcolor{red}{\sim  T \mid \& T} \\
        & D & \Coloneqq & \text{datatype or interface names} \\
        & C & \Coloneqq & \text{constructor or generator names} \\
        & f & \Coloneqq & \text{consumer or destructor names} \\
        % \text{Names}
...
\begin{figure}[t]
% \framebox{$\dtrToCsm {Dtr} {Csm}$}
\begin{mathpar}

\inferrule[Dt2It]
{ D \in \dt \\ \overline{f} = \consumer{D} \\ \overline{\Delta; \Gamma \vdash \defs(f) \leadsto Dtr}  \\ \trans L T L'}
{ \trans {\sealed D; L} T {\kwinterface \; D \{ \overline{Dtr}\} }; L'}
 
\inferrule[Ctr2Gen]
{ D \in \dt \\ \overline{f} = \consumer{D} \\ \overline{\Delta;\Gamma,\overline{x: T} \vdash_C {\defs(f)} \leadsto {Fun}}\\ \trans L T L'}
{ \trans {\ctr{C}{D}; L} T \classf{C}{x}{T}{D}{\overline{Fun}}; L'}
% { \textsf{fresh } x \\ \overline{Gen} = \generator{D} \\  }

\inferrule[CsmElim]
{ D \in \ct \\ \trans L T L'}
{ \trans {\csmcase e; L} {T} L'}
\end{mathpar}


% \framebox{$\Delta; \Gamma \vdash Csm \leadsto Dtr$}
\begin{mathpar}
% \inferrule[Csm2Dec]
% { {\kwcase\; \_ \Rightarrow e} \notin \overline{P}}
% {  \Gamma \vdash \csm P \leadsto \dec }

\inferrule[Csm2Fun]
{ {\kwcase\; \_ \Rightarrow e} \in \overline{\case P e} \\ \transCtx{\kwself: D,\overline{x:T}} e T e'}
{  \Delta; \Gamma \vdash \csmcase e  \leadsto \dec x = [\kwself \mapsto \kwthis]e'}

\inferrule[Csm2Dec]
% { {\kwcase\; \_ \Rightarrow e} \notin \overline{P}}
{}
{  \Delta; \Gamma \vdash \csmcase e \leadsto \dec x}
\end{mathpar}
%% \cristina{Do we need Fun2Case as well? I don't fully follow Case2Fun as opposed to Csm2Fun.}
%% \weixin{Rename Gen2Case as Fun2Case}
% \framebox{$\csmToDtr {Csm} {Fun}$}
\begin{mathpar}
\inferrule[Case2Fun]
{ \case {C(\overline{y})}{e}  \in \overline{\case P e} \\ \transCtx{\kwself: D,\overline{x:T}} e T e'}
{ \csmToDtr { \csmcase e} {\dec x = [\kwself \mapsto \kwthis]e'}}
...
\inferrule[CreateRef]
{}
{ \trans{e}{T}{\& e \Rightarrow \& T}}

\inferrule[CreatePointer]
{}
{ \trans{e}{T}{\sim e \Rightarrow \sim T}} \\
...
\end{mathpar}
\caption{Core translation rules for functional decomposition}
\label{fig:trans-fp}
\end{figure}
...
The first stage of this process is to transform the types themselves. The rules for this process are unchanged, however there are some additional considerations in Rust, including visibility and dyn traits. A demo of this transformation stage is shown in \autoref{fig:trans-type-stage}
...
Having transformed the definitions of each type, the next stage of the transformation is to convert the associated functions. For OOP this involves transforming the implementations of the trait methods into consumers, whilst for FP, it involves transforming the consumers into the trait methods and associated implementations for the structs. The rules for these transformations require some minor alteration in order the accommodate the change in type of the function arguments and return types.

As mention in \autoref{sec:dyn}, a reference to a trait cannot be created, without the \rust{dyn} token. This means when converting a method from FP, if a consumer has any function arguments which type match that of the enum, the type must be updated to a \rust{dyn} reference.

The following equivalnce has been created
dyn Box thing -> thing
&thing -> &thing

For OOP to FP this means transforming the traits, structs and impls and for FP to OOP this means transforming the enums and consumers.
